...
const menuLabel = service => service.title;
const compactMeta = text => text.length <= 155 ? text : `${text.slice(0, 151).replace(/\s+\S*$/, '')} …`;
const aiBadgeSvg = `<span class="ai-badge" title="KI-generiertes Bild" aria-label="KI-generiertes Bild"><svg viewBox="89 100 366 366" aria-hidden="true"><circle cx="272" cy="283" r="180" fill="rgba(17, 58, 74, 0.90)" stroke="rgba(255, 255, 255, 0.55)" stroke-width="8"/><g fill="#ffffff"><path d="M170.79,353.74c-1.08,0-2.05-.43-2.92-1.31-.88-.87-1.31-1.84-1.31-2.92,0-.67.07-1.27.2-1.81l47.34-129.32c.4-1.48,1.24-2.79,2.52-3.93,1.27-1.14,3.05-1.71,5.34-1.71h29.81c2.28,0,4.06.57,5.34,1.71,1.27,1.14,2.11,2.45,2.52,3.93l47.14,129.32c.27.54.4,1.14.4,1.81,0,1.08-.44,2.05-1.31,2.92s-1.91,1.31-3.12,1.31h-24.78c-2.01,0-3.52-.5-4.53-1.51-1.01-1.01-1.65-1.91-1.91-2.72l-7.86-20.55h-53.78l-7.65,20.55c-.27.81-.88,1.71-1.81,2.72-.94,1.01-2.55,1.51-4.83,1.51h-24.78ZM218.13,299.96h37.47l-18.93-53.18-18.53,53.18Z"/><path d="M328.11,353.74c-1.48,0-2.69-.47-3.63-1.41-.94-.94-1.41-2.15-1.41-3.63v-130.93c0-1.48.47-2.68,1.41-3.63s2.15-1.41,3.63-1.41h26.99c1.48,0,2.68.47,3.63,1.41.94.94,1.41,2.15,1.41,3.63v130.93c0,1.48-.47,2.69-1.41,3.63-.94.94-2.15,1.41-3.63,1.41h-26.99Z"/></g></svg></span>`;
...
function mapSection(service = null) {
  ...
  const mobileOptions = leipzigMap.features.map(feature => `<option value="${esc(feature.name)}"${feature.name === 'Zentrum' ? ' selected' : ''}>${esc(feature.name)} – ${esc(feature.district)}</option>`).join('');
...
function baseSchema(pathname) {
  return {
    ...
    name: site.name, legalName: site.company, url: site.baseUrl, image: absolute('/images/reinigungsfirma-leipzig-professionell.webp'),
...
function overviewPage() {
  ...
  const description = 'Alle Reinigungsleistungen in Leipzig: Gebäude, Gewerbe, Außenanlagen und Spezialreinigung. Passende Leistung finden und kostenlos anfragen.';
...
function industriesPage() {
  ...
  const description = 'Reinigung für Büros, Praxen, Handel, Gastronomie, Fitness, Bildung und Hausverwaltungen in Leipzig. Branchenlösung kostenlos anfragen.';
...
function industryPage(industry) {
  ...
  const industryTitles = {
    ...
    'pflege-soziales': 'Reinigung Pflege & Soziales Leipzig | ReinigungsProfi',
    'industrie-produktion': 'Industriereinigung Leipzig | ReinigungsProfi',
    'logistik-lager': 'Lagerreinigung Leipzig | ReinigungsProfi',
    'auto-werkstatt': 'Autohaus-Reinigung Leipzig | ReinigungsProfi',
    'oeffentliche-einrichtungen': 'Reinigung Verwaltung Leipzig | ReinigungsProfi',
    'kultur-veranstaltung': 'Eventreinigung Leipzig | ReinigungsProfi',
    'bau-handwerk': 'Reinigung Handwerksbetriebe Leipzig | ReinigungsProfi',
    lebensmittelhandel: 'Marktreinigung Leipzig | ReinigungsProfi'
  };
  const title = industryTitles[industry.key] || compactMeta(`${industry.title} Reinigung Leipzig | ReinigungsProfi`);
  const industrySpecific = industry.context ? `<section class="section section--soft"><div class="container content-grid"><div class="content-copy"><span class="eyebrow">Branchentypischer Bedarf</span><h2>Was bei ${esc(industry.title)} besonders zu berücksichtigen ist</h2><p>${esc(industry.context)}</p><p>Vor der Kalkulation prüfen wir deshalb Nutzung, Flächentypen, Zugänge, Betriebszeiten und vorhandene Vorgaben. So werden laufende Aufgaben, stark beanspruchte Zonen und periodische Arbeiten verständlich voneinander getrennt.</p></div><aside class="aside-box"><h3>Darauf achten wir bei der Planung</h3><ul class="check-list">${industry.planning.map(item => `<li>${esc(item)}</li>`).join('')}</ul><a class="btn btn-brand" href="#kontakt">Bedarf besprechen</a></aside></div></section>` : '';
...
function aboutPage() {
  ...
  return `${head({ title: 'Über uns | ReinigungsProfi Leipzig', description: 'ReinigungsProfi Leipzig (Quadclean GmbH): Neugegründete Gebäudereinigung in Leipzig mit modernsten Standards, festen Ansprechpartnern und fairem Service.', pathname, schema, image: '/images/reinigungsfirma-leipzig-ueber-uns-team.webp' })}<body>${header()}${breadcrumb(crumbs)}<main id="main"><section class="hero"><div class="container hero-grid"><div><span class="eyebrow" style="color:#9fd0d7">Neu gegründet in Leipzig</span><h1>Über uns – Ihr professioneller Reinigungspartner in Leipzig</h1><p>ReinigungsProfi Leipzig steht unter dem Dach der neugegründeten Quadclean GmbH für zeitgemäße Qualitätsstandards, verlässliche Betreuung und transparente Absprachen. Als engagiertes Leipziger Reinigungsunternehmen schaffen wir Sauberkeit und Werterhalt für gewerbliche und private Objekte.</p><div class="actions"><a class="btn btn-primary" href="#arbeitsweise">Unsere Arbeitsweise</a><a class="btn btn-secondary" href="#kontakt">Kontakt aufnehmen</a></div></div><div class="hero-media"><img src="/images/reinigungsfirma-leipzig-ueber-uns-team.webp" width="720" height="540" fetchpriority="high" alt="Team und Arbeitsweise von ReinigungsProfi Leipzig">${aiBadgeSvg}</div></div></section>${trustStrip()}<section class="section" id="arbeitsweise"><div class="container"><div class="section-head center"><span class="eyebrow">Was uns wichtig ist</span><h2>Moderne Standards, persönliche Betreuung, verlässliche Leistung</h2><p>Sauberkeit ist unsere Leidenschaft. Als neugegründetes Reinigungsunternehmen verbinden wir frische Motivation und aktuelle Branchenstandards mit solider Verantwortung und direkten Kommunikationswegen.</p></div><div class="feature-grid"><article class="feature"><h3>Neu gegründet & engagiert</h3><p>Wir starten mit modernen Prozessen, transparenter Kommunikation und dem Ziel, Gebäudereinigung in Leipzig partnerschaftlich und unkompliziert zu gestalten.</p></article><article class="feature"><h3>Feste Ansprechpartner & kurze Wege</h3><p>Ob Geschäftsführung oder Objektleitung – bei Fragen und Wünschen erreichen Sie direkt kompetente Kontakte ohne zeitraubende Umwege.</p></article><article class="feature"><h3>Echte Wertschätzung & feste Teams</h3><p>Eigene sozialversicherungspflichtig angestellte Mitarbeiter, faire Bedingungen und professionelle Ausrüstung sichern erstklassige Reinigungsergebnisse.</p></article></div></div></section><section class="section section--soft"><div class="container content-grid"><div class="content-copy"><span class="eyebrow">Unsere Arbeitsweise</span><h2>Erst verstehen, dann ein passendes Angebot erstellen</h2><p>Jedes Objekt hat eigene Abläufe, Materialien und Nutzungszeiten. Deshalb sprechen wir vor jedem Angebot ausführlich über Ihre Anforderungen: Welche Flächen werden wie intensiv genutzt, welche Zeitfenster sind ideal und worauf kommt es Ihnen besonders an? Bei größeren Flächen besichtigen wir das Objekt persönlich vor Ort.</p><p>Das Ergebnis ist ein verständliches, faires Leistungsverzeichnis. Es trennt laufende Unterhaltsarbeiten klar von periodischen Aufgaben wie Fenster-, Grund- oder Außenreinigungen. So behalten Sie stets die volle Kostenkontrolle ohne versteckte Überraschungen.</p><p>Ehrlichkeit gehört zu unseren Grundwerten: Wir sagen offen, welche Leistungen realistisch und wirtschaftlich sind. Bei speziellen Gefahrstoffen oder zulassungspflichtigen Aufgaben benennen wir Grenzen klar und empfehlen bei Bedarf geeignete Fachpartner.</p></div><aside class="aside-box"><h3>Quadclean GmbH</h3><p><strong>Geschäftsführer:</strong> ${site.managingDirector}<br><strong>Prokurist:</strong> ${site.procurator}<br><strong>Standort:</strong> ${site.address}, ${site.postalCode} ${site.city}<br><strong>Telefon:</strong> ${site.phoneDisplay}<br><strong>E-Mail:</strong> ${site.email}</p><a class="btn btn-brand" href="/dienstleistungen/">Leistungen entdecken</a></aside></div></section><section class="section"><div class="container content-grid"><div class="content-copy"><span class="eyebrow">Führung & Team</span><h2>Verlässliche Qualität durch erfahrene Führung und motivierte Mitarbeiter</h2><p>Hinter der Quadclean GmbH steht ein engagiertes Team: Geschäftsführer Slatan Nadarevic und Prokurist Sven Schicke leiten das Unternehmen mit klarer Struktur, hohem Anspruch an Gründlichkeit und ausgeprägter Kundenorientierung.</p><p>Unsere Mitarbeiter werden objektbezogen eingewiesen, kontinuierlich geschult und arbeiten mit hochwertigen, materialschonenden Reinigungsmitteln. Wer eine verlässliche Anstellung in einem fairen Arbeitsumfeld sucht, findet auf unserer Jobseite aktuelle Einstiegsmöglichkeiten.</p><a class="btn btn-brand" href="/jobs/">Jobs bei uns ansehen</a></div><aside class="aside-box"><h3>Direkter Kontakt</h3><p>Sie möchten die Quadclean GmbH kennenlernen oder Ihr Objekt besprechen? Rufen Sie uns an oder senden Sie eine kurze Anfrage.</p><a class="btn btn-brand" href="tel:${site.phone}">${site.phoneDisplay}</a></aside></div></section>${contactSection()}</main>${footer()}</body></html>`;
}
...
function homePage() {
  ...
  <section class="hero"><div class="container hero-grid"><div><span class="eyebrow" style="color:#9fd0d7">Lokal. Persönlich. Klar organisiert.</span><h1>Ihre Reinigungsfirma in Leipzig – zuverlässig, persönlich, professionell</h1><p>ReinigungsProfi Leipzig betreut Büros, Praxen, Wohnanlagen, Gewerbeobjekte und Außenanlagen in Leipzig. Sie erhalten einen festen Ansprechpartner, ein verständliches Angebot und einen Ablauf, der zu Ihrem Objekt passt.</p><div class="actions"><a class="btn btn-primary" href="#kontakt">Kostenloses Angebot anfordern</a><a class="btn btn-secondary" href="tel:${site.phone}">Jetzt anrufen</a></div></div><div class="hero-media"><img src="/images/reinigungsfirma-leipzig-professionell.webp" width="720" height="540" fetchpriority="high" alt="Professionelle Gebäudereinigung durch ReinigungsProfi Leipzig">${aiBadgeSvg}</div></div></section>${trustStrip()}
  <section class="section section--soft" id="leistungen"><div class="container"><div class="section-head center"><span class="eyebrow">Alle ${services.length} Leistungen</span><h2>Unsere Reinigungsleistungen in Leipzig</h2><p>Wählen Sie direkt die passende Leistung. Jede Reinigung führt zu einer eigenen Seite mit Leistungsumfang, Ablauf, häufigen Fragen und Anfrageformular.</p></div><div class="card-grid four">${services.map(serviceCard).join('')}</div><div class="actions"><a class="btn btn-brand" href="/dienstleistungen/">Zur ausführlichen Leistungsübersicht</a></div></div></section>
...
function notFoundPage() {
  ...
  const schema = { '@context': 'https://schema.org', '@type': 'WebPage', name: title, description, url: absolute(pathname), isPartOf: { '@id': `${site.baseUrl}/#business` } };
